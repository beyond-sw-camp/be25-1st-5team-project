...
| <img src="./image/zootopia_rat.jpg" width="150"><br>**윤정윤** | <img src="./image/zootopia_judy.jpg" width="150" height="150"><br>**이용호** | <img src="./박재하/profile_J.jpg" width="150"><br>**박재하** |
| :---: | :---: | :---: |
| **팀원** | **팀원** | **팀원** |
...
- 팀원 관리 권한: 강제 퇴장 및 페널티 부여 (부적절/불성실 멤버 강제 탈퇴 처리 가능,
...
```sql
-- ===================== LEADER_002 =====================
-- 모집중인 스터디가 삭제되거나 취소하는 경우 연관된 데이터들을 정리
-- 데이터의 무결성을 위함
-- 북마크는 삭제
-- 스터디 멤버들은 CANCELED 처리함
-- 이력을 남기기 위해 삭제하지 않고 소프트 딜리트 함
...
### 🌱 이다윗
>  데이터베이스 프로젝트는 처음이었으나, 팀원들과의 적극적인 협업을 통해 많은 것을 배울 수 있었습니다.
>  GitHub 기반의 협업 워크플로우와 컨벤션을 준수하며 실무 감각을 익혔고, Master/Slave 아키텍처 등 새로운 기술 개념을 습득하는 과정에서 큰 흥미를 느꼈습니다.
>  기술적 성장뿐만 아니라, 팀원들과의 원활한 소통을 통해 문제를 해결해 나가는 협업 능력 또한 기를 수 있었던 소중한 경험이었습니다.
...
### 🔍 이애은
>  이번 팀 프로젝트에서는 Master/Slave 구조에서 DB 작업을 진행하며, 실제 서비스 환경에서 데이터베이스가 어떻게 운영되는지 이해할 수 있었습니다.
>  DB 단에서 서비스 흐름을 명확히 보여주기 위해 이론으로만 배웠던 프로시저와 트리거를 활용하고
>  이를 적용하는 과정에서 단순한 쿼리 작성 이상의 고민이 필요하다는 것을 느꼈습니다.
>  특히 트리거를 구현하면서 테이블 간의 연관성과 데이터 흐름을 계속해서 고려하게 되었고 이를 통해 구조 설계의 중요성을 체감했습니다.
>  또한 팀원들과의 협업 과정에서 Git 충돌이 발생했지만, 기존에 공부했던 충돌 해결 방법을 직접 적용하여 문제를 해결하며 Git을 활용한 협업에 대한 이해도 높일 수 있었습니다.
> 이번 프로젝트는 DB 설계 경험과 협업 역량을 함께 성장시킬 수 있었던 의미 있는 경험이었습니다. 

### 🌿 이용호
>  이번 팀 프로젝트를 통해 데이터베이스 설계는 서비스의 요구사항과 로직을 구조적으로 정리하고 시스템 전체 흐름을 설계하는 핵심 과정이라는 점을 이해했습니다.
>  요구사항 정의서에서 ERD, 테이블 정의서, 쿼리 및 프로시저로 이어지는 흐름을 따라가며 각 단계가 서로 긴밀하게 연결되어 있다는 점을 인식하게 되었습니다. 단순 조회는 일반 쿼리로 충분하지만, 상태 변화와 조건 분기가 필요한 기능은 데이터베이스 차원에서 로직을 명확히 표현해야 한다는 점을 배웠습니다. 
>  또한 Git을 활용한 팀 협업 과정에서 브랜치 작업 중 발생한 충돌과 변경 이력 혼선을 해결하기 위해 역할별 브랜치 분리와 커밋 규칙을 정리했고, 이를 통해 Git은 팀원 간 작업 흐름과 설계 의도를 공유하는 중요한 수단이라는 점을 깨달았습니다.
>  이번 경험이 향후 백엔드 개발과 팀 프로젝트에서 보다 구조적이고 책임감 있는 설계를 수행하는 데 중요한 기반이 될 것이라고 생각합니다.


### ✨ 윤정윤
>  이번 프로젝트를 진행하면서 가장 좋았던 점은 팀원 간 역할 분배가 효율적으로 이루어졌고, 각 팀원이 프로젝트 주제에 대해 충분히 이해하고 적극적으로 참여했다는 점입니다.
>  덕분에 협업 과정에서 혼선이 적었고 전체적인 개발 흐름도 안정적으로 유지할 수 있었고 회의 후 항상 유의미한 결론를 도출해냈습니다.
> 
>  또한 이번 프로젝트는 애플리케이션 서버 없이 오로지 DB만으로 서비스를 구현해본다는 새로운 관점의 도전이었는데,
>  이를 통해 실제 서비스 환경에서 데이터가 어떻게 관리되고 운영되는지에 대해 더 깊게 고민할 수 있었습니다.
>  특히 장애 복구 구조와 읽기/쓰기 분산 시스템을 직접 구현해보면서 master-slave 구조의 동작 원리와 운영 관점에서 고려해야 할 요소들을 실습을 통해 확실하게 학습할 수 있었습니다.
> 
>  아쉬웠던 점은 초기 기술 선택 단계에서 slave 자동 승격(장애 복구)까지 고려하지 못했다는 것입니다.
>  당시에는 쿼리 분산을 구현하는 것에 집중해 ProxySQL로 읽기/쓰기 분기 구조를 먼저 구성했지만, 프로젝트를 진행하면서 장애 복구의 필요성을 뒤늦게 인지했고 그 과정에서 MaxScale을 알게 되어 도입하게 되었습니다.
>  문제는 MaxScale 자체에도 readwritesplit 기반 쿼리 분기 기능이 존재한다는 점이었는데, 이 사실을 초반에 알았다면 ProxySQL을 별도로 사용할 필요 없이 MaxScale 중심으로 더 단순한 구조를 설계할 수 있었습니다.
>  결과적으로 기술 스택이 불필요하게 복잡해졌다는 점이 가장 아쉬움으로 남았습니다.


### ✏️ 박재하
>   처음 진행하는 프로젝트라 어려움이 많았지만 배움도 많았습니다.
>  데이터베이스 설계부터 완성하는 과정을 통해 탄탄한 기본기를 다질 수 있었습니다. 
>  요구사항 명세서 작성부터 ERD 설계, 테이블 명세서 및 DDL 생성, 그리고 최종 테스트에 이르는 DB 설계의 전체 과정을 직접 수행하며 데이터 구조에 대해 이해할 수 있었습니다.
>  이 과정에서 팀원의 작업내용이 내가 작업한 내용에 어떤 영향을 미치는지 파악하며 데이터 흐름과 모듈간 의존성을 확인할 수 있었습니다.
>  프로젝트를 통해 수업시간에 배운 트리거와 프로지서를 활용하여 작업을 완수함으로써 배웠던 내용을 적용해 볼 수 있는 시간이었습니다.
>
>  또한 팀원들과 문서를 동시 편집하는 과정에서 발생한 Git 충돌 문제를 해결하며 파일의 형상관리와 협업 과정을 간접적으로 배울 수 있었습니다.
>  이번 경험은 단순한 기술 습득을 넘어, 차기 프로젝트의 완성도를 높이는데 기여할 중요한 밑거름이 될 것입니다.
